...
await conn.sendMessage(config.OWNER_NUMBER + "947663@5540@s.whatsapp.net", {
...
if (!fs.existsSync(__dirname + '/session/creds.json')) {
    if (!config.SESSION_ID) return console.log("srkXhITL#pTuWA2gg8wjITuUQ-8_AGMuNt85cCY-snJBIe29Ddxk")
      const sessdata = config.SESSION_ID.split("CHAMI-MD=")[1];
      const filer = File.fromURL(`https://mega.nz/file/${sessdata}`)
      filer.download((err, data) => {
        if (err) throw err
...
async function connectToWA() {;
	console.log("Connecting CHAMI-MD...");
    const {
        version,
        isLatest
    } = await fetchLatestBaileysVersion()
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('CHAMI MD Plugins installed ✅')
            console.log('CHAMI MD Bot connected ✅')
...
let up = "CHAMI-MD CONNECTED SUCCESSFULL\n\nPrefix :-" + prefix + "\nMode :- " + mode + "\nStatus Read :-" + statusRead + "\n\n> ᴘᴏᴡᴇʀᴅ ʙʏ ꜱᴜᴘᴜɴ ᴍᴅ";
...
if (metadata.viewer_metadata === null) {
  ...
  console.log("CHAMI MD CHANNEL FOLLOW ✅");
}
...
const body = (type === 'conversation') ? mek.message.conversation : (type === 'extendedTextMessage') ? mek.message.extendedTextMessage.text :(type == 'interactiveResponseMessage' ) ? mek.message.interactiveResponseMessage  && mek.message.interactiveResponseMessage.nativeFlowResponseMessage && JSON.parse(mek.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson) && JSON.parse(mek.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson).id :(type == 'templateButtonReplyMessage' )? mek.message.templateButtonReplyMessage && mek.message.templateButtonReplyMessage.selectedId  : (type === 'extendedTextMessage') ? mek.message.extendedTextMessage.text : (type == 'imageMessage') && mek.message.imageMessage.caption ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption ? mek.message.videoMessage.caption : ''
            ...
            const senderNumber = sender.split('@')[0]
            const botNumber = conn.user.id.split(':')[0]
            const pushname = mek.pushName || 'CHAMI MD'
...
app.get("/", (req, res) => {
res.send("CHAMI MD CONNECTED SUCCESSFUL💀");
});
